...
filename = strcat(output_dir, 'matrices/rmse_mat_withSmallerN.mat');
% save(filename, 'rmse_mat', '-v7.3')
...
n_list = [50 60 70 80 90 100 120 140 150 200 300 500];
...
for i=1:length(n_list)
    ...
    % Plot RMSE for N
    p = plot(rmse_mat_subset(:,8), rmse_mat_subset(:,7), '-ok', "LineWidth", 2, 'DisplayName', strcat('N=', num2str(n)));
    p.Color = color_map(i,:);

    % Add RMSE for hierarchal model as horizontal line
    p = yline(rmse_hi, '--', "LineWidth", 2, 'DisplayName', strcat('N=', num2str(n), ', \lambda=0'));
    p.Color = color_map(i,:);
end
hold off
...
plot_filename = output_dir + "plots/avg_rmse_bw_scaled_probability_curves_throughE=4_withSmallerN.png";
exportgraphics(fig, plot_filename, 'Resolution', 300)
