...
for i_para = 1:length(epsilon_list)
    ...
    disp("Currently processing epsilon " + epsilon + " and lambda " + lambda);
...
%Reinitialize data matrix for every epsilon
    ...
    % col1 = clone cell belongs to, col2 = competitive or master
    % col2 = memory retained (binary), col3 = number of iterations retained
    ...
    %lists of the probabilities that each clone will be selected for proliferation/differentiation.
        f_pro_list=(lambda*x_temp+epsilon + memory_strength*mem_matrix(:,1))/(lambda*n_openniche+num_of_clones*epsilon+(memory_strength*sum(mem_matrix(:,1))));
    ...
    % 5. Assign memory to a cell
               ...
               mem_matrix(mem_matrix(:,1) == pro_type & mem_matrix(:,2) == 0, 3) = 1; % yes, memory
               mem_matrix(mem_matrix(:,1) == pro_type & mem_matrix(:,2) == 0, 4) = 1; % first iteration
            else
    ...
    filename = ['results/hierarchal_neutral_competition/memory/matrices/timeseries', run_tag , '.mat'];
    save(filename, 'x_matrix')
    save(strcat('results/hierarchal_neutral_competition/memory/matrices/variables',run_tag,".mat"))
end
